...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/10/21 15:05:09 by romargar          #+#    #+#             */
/*   Updated: 2025/10/21 15:18:42 by romargar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "philosophers.h"

static int	check_if_philo_died(t_shared *shared, int i)
{
...
static int	check_for_optional_argument(t_shared *shared)
{
	int	count;
	int	i;
	...
	{
		if (check_if_philo_died(shared, i) == 1)
			return (1);
		pthread_mutex_lock(&shared->last_meal_mutex);
...
void	*death_monitor(void *arg)
{
	...
	while (1)
	{
		...
		{
			if (check_for_optional_argument(shared))
				return (NULL);
		}
		usleep(1000);
	}
